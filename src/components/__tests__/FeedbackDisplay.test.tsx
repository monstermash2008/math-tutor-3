...
describe("FeedbackDisplay Component", () => {
	it("should display a loading state", () => {
		render(
			<FeedbackDisplay status="loading" message="Checking your answer..." />,
		);
	...
	it("should display a warning message", () => {
		render(
			<FeedbackDisplay
				status="warning"
				message="Please simplify your answer."
			/>,
		);
	...
	it("should render nothing when status is idle", () => {
		const { container } = render(
			<FeedbackDisplay status="idle" message="Should not show" />,
		);
	...
	it("should render nothing when message is empty", () => {
		const { container } = render(
			<FeedbackDisplay status="success" message="" />,
		);
		expect(container.firstChild).toBeNull();
	});
	...
	it("should apply correct CSS classes for different states", () => {
		const { rerender } = render(
			<FeedbackDisplay status="success" message="Success message" />,
		);
	...
	it("should not display prompt debug section when no prompt is provided", () => {
		...
		expect(
			screen.queryByText("🔍 Debug: View LLM Prompt"),
