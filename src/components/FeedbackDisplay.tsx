...
export function FeedbackDisplay({
		...
		switch (status) {
			case "loading":
				return {
					...
					icon: (
						<svg
							className="w-8 h-8 text-blue-500 mr-4 flex-shrink-0 animate-spin"
							fill="none"
							viewBox="0 0 24 24"
							aria-label="Loading"
						>
	...
	return (
		<div className="mt-6">
			<div
				className={`feedback-card ${bgColor} p-5 rounded-lg border flex items-start shadow-sm transition-all duration-300`}
			>
				{icon}
				<div>
				...
				<div className="mt-4 bg-gray-50 border border-gray-200 rounded-lg">
					...
					>
						<div className="flex items-center justify-between">
							...
							<span
								className={`transform transition-transform ${showPrompt ? "rotate-90" : ""}`}
							>
								▶️
							</span>
						</div>
